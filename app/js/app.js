...
async function updateList (event) {
  if (event.keyCode === 13) {
    ...
    await fetchData(
      `${baseUrl}list/${event.target.id.slice(2)}`,
      'PUT',
      'application/json',
      JSON.stringify({
        listName
...
async function addtodo (event) {
  if (event.keyCode === 13) {
    if (event.target.value === '') return
    const response = await fetchData(
      baseUrl + 'todo/' + event.target.id.slice(4),
      'POST',
      'application/json',
      JSON.stringify({
...
async function updateTask (event) {
  if (event.keyCode === 13) {
    ...
    const response = await fetchData(
      baseUrl + 'todo/' + event.target.parentNode.parentNode.parentNode.id.slice(4) + '/' + event.target.id.slice(2),
      'PUT',
      'application/json',
      JSON.stringify({
        column: 'todoname',
...
async function textinput (event) {
  const id = event.target.id.slice(2)
  const response = await fetchData(
    baseUrl + 'todo/' + event.target.parentNode.parentNode.parentNode.id.slice(4) + '/' + id,
    'PUT',
    'application/json',
    JSON.stringify({
      column: 'note',
...
async function schedule (event) {
  ...
  let todos = await fetchData(
    baseUrl + 'todo/' + todoContainer.id.slice(2),
    'GET',
    'application/json'
  )
...
async function todoComplete (event) {
  ...
  if (document.getElementById('tx' + id).style.textDecoration === 'none') {
    ...
    await fetchData(
      baseUrl + 'todo/' + parent.parentNode.id.slice(4) + '/' + id,
      'PUT',
      'application/json',
      JSON.stringify({
        column: 'completed',
